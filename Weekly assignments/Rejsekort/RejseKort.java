public class RejseKort {
    ...
    public void checkIn(int x, int y, int timeStamp) {
        if (balance >= 100) {
            if (isCheckedIn(timeStamp)) {
                System.out.println(
                        "Continued journey (" + getTimeSinceLastCheckIn(timeStamp) + " minutes since last check in)");
            } else {
                checkedIn = true;
                maxX = x;
                minX = x;
    ...
    public void checkOut(int x, int y, int timeStamp) {
        ...
        if (checkedIn && isCheckedIn(timeStamp)) {
            ...
            System.out.println(
                    "Checked out! " + getTimeSinceLastCheckIn(timeStamp) + " minutes since last check in. Price is "
                            + price + " DKK, remaining balance is " + balance + " DKK");
    ...
    private int getTimeSinceLastCheckIn(int timeStamp) {
        if (checkedInTimeStamp == -1) { // No check-in has been recorded
            return Integer.MAX_VALUE; // Use a large number to represent no valid check-in time
        }
